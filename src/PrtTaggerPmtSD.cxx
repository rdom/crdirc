...
TaggerPMTSD::TaggerPMTSD(G4String name) : G4VSensitiveDetector(name) {
...
void TaggerPMTSD::Initialize(G4HCofThisEvent *hce) {
  ...
  fHitsCollection = new PrtTaggerPmtHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool TaggerPMTSD::ProcessHits(G4Step *step, G4TouchableHistory *) {
  ...
  auto hit = new PrtTaggerPmtHit(copyNo);
