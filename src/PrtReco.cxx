...
PrtReco::PrtReco(TString infile, TString lutfile, TString pdffile, TString nnfile, int verbose) {
  ...
  fTimeProp = new TH1F("timeprop", ";measured time [ns];entries [#]", 1000, 0, 30);
  fTimeDiff = new TH2F("timediff", ";measured time [ns];t_{meas}-t_{calc} [ns]", 500, 0, 30, 150, -5, 5);
  ...
  fCtr = new TH1F("ctr", ";#Delta#theta_{C} [mrad];entries [#]", 150, -15, 15);
  ...
  for (int h = 0; h < 5; h++) {
    hthetac[h] = new TH1F(Form("thetac_%d", h), ";#theta_{C} [rad];entries [#]", 100, 0.75, 0.9);
    hthetacd[h] =
...
void PrtReco::Run(int start, int end) {
  ...
  double cangle[5] = {0}, spr[5] = {0}, trr[5] = {0}, nph_gr[5] = {0}, nph_gr_err[5] = {0},
         ...
         track_res1(0), track_res2(0), ctr_sigma(0), ctr_err(0);
  ...
  for (int ievent = start; ievent < nEvents && ievent < pdfstart; ievent++) {
    ...
    mom_vertex.RotateY(TMath::TwoPi() - fTheta * TMath::DegToRad());
 
    // track already smeared during simulation at tracking layer.
    mom_vertex.SetTheta(gRandom->Gaus(mom_vertex.Theta(), trackingResTheta));
    mom_vertex.SetPhi(gRandom->Gaus(mom_vertex.Phi(), trackingResPhi));
    
    fpid = 1; //ft.get_pid(fEvent->getPid());
    ...
    if (fMethod == 3) {
      ...
      if (1) {
        FindPeak(cangle, spr);
        fCtr->Fill(1000 * (cangle[1] - fAngle[1]));
	hthetac[fp1]->Reset();
	hthetac[fp2]->Reset();
      }
    }
...
void PrtReco::FindPeak(double (&cangle)[5], double (&spr)[5]) {
  for (int h = 0; h < 5; h++) {
    ...
    if (hthetac[h]->Integral() > 20) {
      ...
      hthetac[h]->Fit("fgaus", "Q", "", 0.78,0.86);
      fFit->ReleaseParameter(2); // width
      // hthetac[h]->Fit("fgaus", "MQ", "", cangle[h] - 3.5 * fSigma[h], cangle[h] + 3.5 * fSigma[h]);
      hthetac[h]->Fit("fgaus", "MQ", "", 0.78,0.86);
      cangle[h] = fFit->GetParameter(1);
      spr[h] = fFit->GetParameter(2) * 1000;

      if (fMethod == 3) {
	// fFit->SetParLimits(1, fAngle[fp1] - 0.01, fAngle[fp2] + 0.01);
	fFit->SetParLimits(2, 0.003, 0.007);
        fFit->SetParameters(10, 0.5 * (fAngle[fp1] + fAngle[fp2]), 0.006);
        gROOT->SetBatch(1);
        hthetac[h]->Fit("fgaus", "LQ", "", 0.6, 1);
        cangle[h] = fFit->GetParameter(1);
        spr[h] = fFit->GetParameter(2);
        ...
        if (fVerbose > 2) {
          ...
          // hthetac[h]->SetTitle(Form("%d", tofpdg));
          // if (tofpdg == 2212) hthetac[h]->SetLineColor(kRed);
          // else hthetac[h]->SetLineColor(kBlue);
          hthetac[h]->Draw("h");
          drawTheoryLines(fMomentum);
...
double PrtReco::FindStartTime(PrtEvent *evt) {
  ...
  for (PrtHit hit : evt->getHits()) {
    ...
    //======================================== dynamic cuts
    {
      { // time cuts
        if (fTheta <= 75) {
          if (htime < 7 || htime > 45) continue;
          reflected = true;
        } else if (fTheta > 105) {
          if (htime < 3 || htime > 40) continue;
          reflected = false;
        } else {
          if (htime < 11) reflected = false;
          else reflected = true;
        }
      }
    }
...
void PrtReco::geom_reco(PrtEvent *event, TVector3 mom, bool ringfit) {
  ...
  for (auto hit : event->getHits()) {
    ...
    if (isGoodHit_gr || (!fGeomReco)) {
      ...
      } else if (pid == 1) ft.fill_digi(mcp, pix);
    }
  }
